 ...
 */
var ASCII_LETTER = Symbol('ASCII');
var OCCIDENTAL_LETTER = Symbol('OCCIDENTAL');
var UTF8_LETTER = Symbol('UTF8');
...
function letter() {
    ...
    if (symbol === null || symbol === OCCIDENTAL_LETTER) {
        return _flowBundle2.default.satisfy(function (c) {
            return 'a' <= c && c <= 'z' || 'A' <= c && c <= 'Z' || isExtendedOccidental(c);
        });
    }
    ...
    if (symbol === UTF8_LETTER) {
        return _flowBundle2.default.satisfy(function (c) {
            return 'a' <= c && c <= 'z' || 'A' <= c && c <= 'Z' || isUtf8Letter(c);
        });
    }
...
function utf8Letter() {
    return _flowBundle2.default.satisfy(isUtf8Letter);
}

function emoji() {
    return _flowBundle2.default.satisfy(function (c) {
        return !isUtf8Letter(c) && isEmojiRegex(c);
    }).rep();
}

function letters(symbolOrTestFunctionOrRegex) {
    return letter(symbolOrTestFunctionOrRegex).rep().map(function (values) {
        return values.join('');
    });
}
...
function stringIn(array) {
    var tryString = function tryString(s) {
        return _flowBundle2.default.try((0, _flowBundle.string)(s));
    };

    if (array.length === 0) {
        return _flowBundle2.default.nop();
    }
    if (array.length === 1) {
...
function notString(s) {
    return _flowBundle2.default.not((0, _flowBundle.string)(s));
}

// unit -> Parser string char
function stringLiteral() {
    var anyChar = (0, _flowBundle.string)('\\"').or(notChar('"'));
...
exports.default = {
    ...
    lowerCase: lowerCase(),
    upperCase: upperCase(),
    UTF8_LETTER: UTF8_LETTER,
    OCCIDENTAL_LETTER: OCCIDENTAL_LETTER,
