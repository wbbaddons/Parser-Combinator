...
exports.default = {
    ...
    'expect (opt) to come back if fail': function expectOptToComeBackIfFail(test) {
        test.expect(1);
        // tests here
        test.equals(_index3.C.char('b').then(_index3.C.string('aaFAIL').opt().drop()).then(_index3.C.string('aaab')).parse(_index2.default.ofString('baaab')).value.join(''), 'baaab');
        test.done();
    },
